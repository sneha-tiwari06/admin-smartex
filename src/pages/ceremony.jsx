...
const CeremonyHome = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axiosInstance.get(`/ceremony`);
        setPosts(res.data);
      } catch (err) {
        console.log(err);
      }
  ...
  const handleDelete = async () => {
    try {
      await axiosInstance.delete(`/ceremony/${postToDelete.id}`);
  ...
  const handleToggleStatus = async (post) => {
    try {
      ...
      await axiosInstance.put(`/ceremony/${post.id}`, { active: updatedPost.active });
